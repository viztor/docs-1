...
export const meta = {
  ...
  lastEdited: '2019-08-28T04:15:22.000Z'
}
...
With exported function that has `w http.ResponseWriter, r *http.Request` signature, the Go Builder is used by Now to compile Go Serverless Functions that expose a single HTTP handler, from a `.go` file within an `/api` directory at your project's root.
...
**The entry point of this Builder is a global matching `.go` files** that export a function that implements the `w http.ResponseWriter, r *http.Request` signature. For more information on using this Builder, [see the Go Builder section](#go).
